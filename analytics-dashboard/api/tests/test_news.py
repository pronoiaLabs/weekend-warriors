...
def test_team_param_binds_in_sql(client: TestClient) -> None:
    ...
    assert body["team"] == "KC"
    assert len(body["rows"]) == 15 and all(r["team_label"] == "KC" for r in body["rows"])
