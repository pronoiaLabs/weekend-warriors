...
| `/:sport/explore` | `GET /api/{sport}/explore`, `GET /api/{sport}/explore/{sheet}?where&order&desc&limit&offset` | `app_explore_*` (seven sheets) | the catalog is each sheet's DESCRIBE TABLE typed into grid kinds; a sheet request is one select with equality filters (`where=column:value`, repeatable) and one sort column, both validated against the catalog before any SQL is built, paged with `has_more`; the page's stats, pivot and TSV copy work over the loaded rows |
