...
-- Purpose : NFL run summary. DLT_DB.OPS.V_TASK_RUNS plus the row counts and status
--           dlt recorded for itself in NFL_PROD_DB.OPS._DLT_RUNS.
-- Run as  : DLT_LOADER_ROLE.
-- Prerequisites : ops/04_run_summary.sql, and at least one completed NFL run so
...
CREATE OR REPLACE VIEW NFL_PROD_DB.OPS.V_PIPELINE_RUNS
    COMMENT = 'One row per NFL pipeline run: task outcome, duration, resources, rows loaded, error.'
AS
WITH dlt_runs AS (
    -- ---------------------------------------------------------------------
    ...
    FROM NFL_PROD_DB.OPS._DLT_RUNS r,
...
-- ---------------------------------------------------------------------
dlt_window AS (
    SELECT MIN(FINISHED_AT) AS DLT_RUNS_FROM FROM NFL_PROD_DB.OPS._DLT_RUNS
)
SELECT
    t.PIPELINE,
    t.TASK_NAME,
...
WHERE t.PIPELINE LIKE 'nfl\_%' ESCAPE '\\';

GRANT SELECT ON VIEW NFL_PROD_DB.OPS.V_PIPELINE_RUNS TO ROLE DLT_DEV_ROLE;
