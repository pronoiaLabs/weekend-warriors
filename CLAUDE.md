...
**dbt model names are project-global**, so only one sport gets the generic names: NFL owns `dim_team` / `fact_team_game` style names (first mover); WNBA and NCAAF core models are sport-qualified (`dim_wnba_team`, `fact_ncaaf_team_game`), matching the `sv_<sport>_*` semantic-view convention. Prep is prefixed per sport everywhere (`stg_nfl__*`, `stg_wnba__*`, `stg_ncaaf__*`).

**Fact tables hold completed games only; the slate lives on the game dimension.** Scheduled games have no fact rows by construction (`fact_team_game` and `fact_wnba_team_game` filter on completedness), `dim_game` / `dim_wnba_game` carry `is_completed`, and each sport's `sv_<sport>_schedule` -- the agents' only future-games tool -- anchors on that dimension with role-playing home/away team joins and deliberately exposes no scores.
