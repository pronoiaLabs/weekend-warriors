...
**The player bridge is a Snowpark procedure dbt calls, not a model.** `dbt-pipelines/snowpark/player_bridge/` is a `snow snowpark` project (`snowflake.yml` generates the `CREATE PROCEDURE`; `make -C dbt-pipelines deploy-snowpark` builds, uploads to `DLT_DB.DEPLOY.SNOWPARK` and re-grants, because `deploy --replace` drops grants exactly like the dbt object). `models/nfl/core/bridges/bridge_player_ids` calls `DLT_DB.DEPLOY.SP_PLAYER_BRIDGE('{{ this.database }}', '{{ this.schema }}', false)` from a pre_hook, so the tables it writes (`PLAYER_BRIDGE`, `PLAYER_BRIDGE_UNMATCHED`) land beside the model in dev and prod alike; `nfl_player_bridge_fqn()` is that address for tests. The procedure maps BallDontLie and Sleeper ids onto nflverse `gsis_id` in tiers (shared ids, then normalized name + team + position group, then name + team with a jersey, college or birth-year tie-break, then batch Cortex Search candidates confirmed by `AI_FILTER`), stores stable ids only so trades change nothing, decides a player once, and returns without touching Cortex when no new player exists, which is what makes the hook free on every build. Unmatched players retry only when their evidence hash changes; `full_refresh => TRUE` by hand re-decides everything. Three vocabularies are mirrored and must move together: `TO_NFLVERSE_TEAM` / `POSITION_GROUPS` in `evidence.py`, `nfl_team_abbr_nflverse` / `nfl_position_group` in `macros/nfl/nfl_bridge_helpers.sql`, and the `POS_GROUP` case in `sql/sources/nfl/09_player_bridge.sql` (the search service's filter attribute). `bridge_game_ids` is plain SQL: nflverse's `season_week_away_home` id observed from play-by-play when played, composed otherwise (LA/LAR, WAS/WSH; BDL playoff weeks 1, 2, 3, 5 are nflverse 19 to 22), with a test that the two forms agree.
