...
/*
    fact_nextgen_receiving -- NFL Next Gen Stats receiving, weekly, from
    nflverse. Grain: player (gsis) x season x season_type x week. Same
    conventions as fact_nextgen_passing: week-0 season totals excluded,
    one-discipline-per-player caveat, game_key / player_game_key via the
    week -> game resolution (LAR -> LA, NGS Super Bowl week 23 -> nflverse
    22), numeric season_type with nflverse_season_type keeping the source's
    REG/POST strings.
*/

with ngs as (

    select * from {{ ref('stg_nfl__nflverse_nextgen_receiving') }}
