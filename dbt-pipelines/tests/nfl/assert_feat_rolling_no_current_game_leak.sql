/*
    Windowed points on a completed RS/post team-game must equal the sum of
    that team's earlier RS/post points in the same season -- never the
    current game.
*/

with base as (

    select
        r.team_game_key,
        r.team_key,
        r.season,
        r.game_datetime,
        r.game_key,
        r.points_scored_std,
        o.points_scored
    from {{ ref('feat_team_game_rolling') }} r
    inner join {{ ref('fact_team_game_offense') }} o
        on r.team_game_key = o.team_game_key
    where r.is_completed
      and r.season_type in (2, 3)

),

expected as (

    select
        a.team_game_key,
        sum(b.points_scored) as expected_points_std
    from base a
    left join base b
        on  a.team_key = b.team_key
        and a.season = b.season
        and (
            b.game_datetime < a.game_datetime
            or (b.game_datetime = a.game_datetime and b.game_key < a.game_key)
        )
    group by a.team_game_key

)

select
    a.team_game_key,
    a.points_scored_std,
    e.expected_points_std
from base a
inner join expected e
    on a.team_game_key = e.team_game_key
where not equal_null(a.points_scored_std, e.expected_points_std)
